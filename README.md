...
- **Pinned** (`lattice-server-linux-x64-v0.3.4`) — immutable, tied to one tag
...
- name: Start LatticeDB
  run: |
    curl -sSfLo lattice-server \
      https://github.com/Avarok-Cybersecurity/lattice-db/releases/download/v0.3.4/lattice-server-linux-x64-v0.3.4
    chmod +x lattice-server
...
Pin a version by swapping `@latest` for `@0.3.4`. `https://unpkg.com/lattice-db@latest/wasm/lattice_server.js`
works the same way.
...
`lattice-db-wasm-v0.3.4.tar.gz` (pinned), verified by `lattice-db-wasm.sha256`.
...
# latticedb-server = { git = "https://github.com/Avarok-Cybersecurity/lattice-db", tag = "v0.3.4", features = ["axum-transport"] }
axum = "0.7"
```
