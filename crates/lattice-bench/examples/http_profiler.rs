...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  LatticeDB: {}", if lattice_available { "✓ Available" } else { "✗ Not running" });
    println!("  Qdrant:    {}", if qdrant_available { "✓ Available" } else { "✗ Not running" });
    println!();
    ...
    println!();
    println!("Baseline HTTP round-trip: {:.2} µs", ping_avg.as_secs_f64() * 1_000_000.0);
    println!();
    ...
    println!("Upsert:");
    println!("  Total RTT:     {:>10.2} µs", upsert_total.as_secs_f64() * 1_000_000.0);
    if let Some(timing) = &upsert_server_timing {
        ...
        if let Some(breakdown) = parse_server_timing(timing) {
            ...
            println!("    Network:     {:>10.2} µs (RTT - server)", network as f64);
        }
    }
    println!();
    ...
    println!("Search:");
    println!("  Total RTT:     {:>10.2} µs", search_total.as_secs_f64() * 1_000_000.0);
    ...
    if qdrant_available {
        ...
        println!("|-----------|-----------|----------|----------|");
        println!("| Ping      | {:>7.2} µs | {:>6.2} µs | {:>+7.2} µs |",
        ...
        println!("| Upsert    | {:>7.2} µs | {:>6.2} µs | {:>+7.2} µs |",
        ...
        println!("| Search    | {:>7.2} µs | {:>6.2} µs | {:>+7.2} µs |",
    ...
    println!("  Baseline (ping): {:.2} µs", ping_avg.as_secs_f64() * 1_000_000.0);
    if let Some(timing) = &upsert_server_timing {
        if let Some(breakdown) = parse_server_timing(timing) {
            let server_overhead = breakdown.total_us - breakdown.handler_us;
            println!("  Server overhead (body+serialize): {:.2} µs", server_overhead as f64);
            let client_overhead = upsert_total.as_micros() as i64 - breakdown.total_us as i64 - ping_avg.as_micros() as i64;
            println!("  Client JSON overhead: ~{:.2} µs", client_overhead.max(0) as f64);
        }
    }
...
fn bench_ping(client: &Client, iterations: usize) -> Result<Vec<Duration>, Box<dyn std::error::Error>> {
...
fn setup_collection(client: &Client, base_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    // Delete if exists
    let _ = client.delete(&format!("{}/collections/bench_profile", base_url)).send();

    // Create collection
    #[derive(Serialize)]
    struct CreateReq { vectors: VectorConfig }
    #[derive(Serialize)]
    struct VectorConfig { size: usize, distance: String }

    let body = CreateReq {
        vectors: VectorConfig { size: VECTOR_DIM, distance: "Cosine".to_string() }
    };

    client.put(&format!("{}/collections/bench_profile", base_url))
        .json(&body)
        .send()?;
    ...
    #[derive(Serialize)]
    struct Point { id: u64, vector: Vec<f32> }

    let points: Vec<Point> = (0..1000).map(|i| {
        Point {
            id: i,
            vector: (0..VECTOR_DIM).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect()
        }
    }).collect();

    client.put(&format!("{}/collections/bench_profile/points", base_url))
...
fn bench_with_timing(
    ...
    struct UpsertReq { points: Vec<Point> }
    #[derive(Serialize)]
    struct Point { id: u64, vector: Vec<f32>, payload: HashMap<String, serde_json::Value> }
    ...
    for i in 0..10 {
        ...
        client.put(&format!("{}/collections/bench_profile/points", LATTICE_URL))
            .json(&UpsertReq { points: vec![point] })
            .send()?;
    }
    ...
    for i in 0..iterations {
        ...
        let start = Instant::now();
        let resp = client.put(&format!("{}/collections/bench_profile/points", LATTICE_URL))
            .json(&UpsertReq { points: vec![point] })
            .send()?;
        total += start.elapsed();
...
fn bench_search_with_timing(
    ...
    #[derive(Serialize)]
    struct SearchReq { vector: Vec<f32>, limit: usize, with_payload: bool, with_vector: bool }
    ...
    // Warmup
    for _ in 0..10 {
        let query: Vec<f32> = (0..VECTOR_DIM).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect();
        client.post(&format!("{}/collections/bench_profile/points/search", LATTICE_URL))
            .json(&SearchReq { vector: query, limit: 10, with_payload: false, with_vector: false })
            .send()?;
    }
    ...
    for _ in 0..iterations {
        ...
        let start = Instant::now();
        let resp = client.post(&format!("{}/collections/bench_profile/points/search", LATTICE_URL))
            .json(&SearchReq { vector: query, limit: 10, with_payload: false, with_vector: false })
            .send()?;
        total += start.elapsed();
...
fn parse_server_timing(header: &str) -> Option<ServerTiming> {
    ...
    Some(ServerTiming { body_us, handler_us, total_us })
}
...
fn setup_qdrant_collection(client: &Client) -> Result<(), Box<dyn std::error::Error>> {
    // Delete if exists
    let _ = client.delete(&format!("{}/collections/bench_profile", QDRANT_URL)).send();
    ...
    client.put(&format!("{}/collections/bench_profile", QDRANT_URL))
        .json(&CreateReq { vectors: VectorConfig { size: VECTOR_DIM, distance: "Cosine".to_string() }})
        .send()?;
    ...
    let points: Vec<Point> = (0..1000).map(|i| {
        Point { id: i, vector: (0..VECTOR_DIM).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect() }
    }).collect();

    client.put(&format!("{}/collections/bench_profile/points", QDRANT_URL))
        .json(&UpsertReq { points })
        .send()?;
...
fn bench_qdrant_health(client: &Client, iterations: usize) -> Result<Duration, Box<dyn std::error::Error>> {
    // Warmup
    for _ in 0..10 {
...
fn bench_qdrant_upsert(client: &Client, iterations: usize) -> Result<Duration, Box<dyn std::error::Error>> {
    #[derive(Serialize)]
    struct UpsertReq { points: Vec<Point> }
    #[derive(Serialize)]
    struct Point { id: u64, vector: Vec<f32> }
    ...
    // Warmup
    for i in 0..10 {
        let point = Point { id: 999_000 + i, vector: (0..VECTOR_DIM).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect() };
        client.put(&format!("{}/collections/bench_profile/points", QDRANT_URL))
            .json(&UpsertReq { points: vec![point] })
            .send()?;
    }
    ...
    let mut total = Duration::ZERO;
    for i in 0..iterations {
        let point = Point { id: 1_000_000 + i as u64, vector: (0..VECTOR_DIM).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect() };
...
fn bench_qdrant_search(client: &Client, iterations: usize) -> Result<Duration, Box<dyn std::error::Error>> {
    #[derive(Serialize)]
    struct SearchReq { vector: Vec<f32>, limit: usize, with_payload: bool, with_vector: bool }
    ...
    for _ in 0..10 {
        let query: Vec<f32> = (0..VECTOR_DIM).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect();
        client.post(&format!("{}/collections/bench_profile/points/search", QDRANT_URL))
            .json(&SearchReq { vector: query, limit: 10, with_payload: false, with_vector: false })
            .send()?;
    }
